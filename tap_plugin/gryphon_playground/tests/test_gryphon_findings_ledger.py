...
def _executor_path() -> Path:
    ...
    return Path(tap_grid.__file__).resolve().parent / "gryphon" / "executor.py"


# Resolve BOTH of these through the package rather than as monorepo-relative paths.
# They used to be `plugins/gryphon_playground/...` and a CWD-relative `tap_grid/...`,
# which silently resolved to nothing once this plugin moved into its own repo — and
# `fl.load` returns [] for a missing file rather than raising, so the breakage
# surfaced as "the ledger is empty" instead of "the ledger is not where I looked".
_LEDGER = Path(fl.__file__).resolve().parent / "gryphon-findings.jsonl"
_EXECUTOR = _executor_path()
